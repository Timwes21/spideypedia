...
import addDetailsToEntries from "./add-details.js";
...
export const Actions = {
    ...
    construct: function(redisPub, chatSession, input){
    ...
    add: async function(key){
        if (key.beingAddedOrRemoved === "issue"){
            try{
                const result = await addDetailsToEntries(key, collection);
        ...
        const result = await collection.updateOne(key.updateAndOption.filter, key.updateAndOption.update);
    ...
    view: async function(agentOutput){
        ...
        const result = await collection.aggregate(aggregates).toArray();
